...
export default function EditDepartment() {
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      setDepLoading(true)
      try {
        const response = await axios.get(`https://backend-site-mongo-db-atlast.vercel.app/api/department/${id}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const response = await axios.put(`https://backend-site-mongo-db-atlast.vercel.app/api/department/${id}`, department, { headers: { "Authorization": `Bearer ${localStorage.getItem('token')}` } })
      if (response.data.success) {
