...
export default function DepartmentList() {
  ...
  const onDepartmentDelete = () => {
    ...
    setDepLoading(true)
    try {
      const response = await axios.get("https://backend-site-mongo-db-atlast.vercel.app/api/department", {
        headers: {
    ...
    <>{depLoading ? <div>Loading...</div> :
      <div className='p-5'>
        ...
        <div className="flex justify-between items-center">
          <input className='px-4 py-0.5 bg-red-100 border' type="text" placeholder='Search by department name' onChange={filterDepartments}/>
