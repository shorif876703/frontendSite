...
export default function LeaveDetails() {
    ...
    const fetchLeave = async () => {
      try {
        const response = await axios.get(`https://employe-backend.vercel.app/api/leave/detail/${id}`, {
          headers: {
  ...
  const changeStatus = async (id, status) => {
    console.log(status)
        try {
        const response = await axios.put(`https://employe-backend.vercel.app/api/leave/${id}`, {status}, {
          headers: {
            Authorization : `Bearer ${localStorage.getItem('token')}`,
          },
        })
    ...
    <div className='max-w-3xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md'>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <img src={`https://employe-backend.vercel.app/${leave.employeId.userId.profileImage}`} className='rounded-full border w-72' />
            </div>
            <div>
